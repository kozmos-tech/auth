...
export interface SignInFormProps extends KozmosClientProps {
  ...
  callbackURL?: string;
  /** Called after a successful sign-in instead of navigating (e.g. router.push). */
  onSuccess?: () => void;
  /** Href of the sign-up page, shown as a footer link when provided. */
  signUpUrl?: string;
...
/**
 * Email/password sign-in card for end-users of a Kozmos project. Works
...
export function SignInForm({
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    setError(null);
    setPending("email");
    const { error } = await actions.signIn.email({ email, password });
    if (error) {
